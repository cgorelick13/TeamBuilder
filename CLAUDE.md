...
## Data Notes
- Fetch all Pokemon names/IDs from PokeAPI on first launch, cache locally with SwiftData
- Fetch full Pokemon data (types, stats, sprite, abilities, evolution chain) on demand, cache after first load
- App should work offline after initial data load
- Show skeleton loading cards during first launch with message: "Downloading Pokemon data — this only happens once."
